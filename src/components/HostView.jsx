...
import {
  doc, setDoc, collection, onSnapshot, updateDoc,
...
export default function HostView({ user, onBack }) {
  const [step, setStep] = useState('compose') // 'compose' | 'room'
  ...
  const [filterQ, setFilterQ] = useState(-1) // -1 = all

  const sessionsPath = (code) => `/artifacts/${APP_ID}/public/data/sessions/${code}`
  const answersColPath = `/artifacts/${APP_ID}/public/data/answers`

  async function generateQuestions() {
    if (!content.trim()) { setGenError('請先輸入共讀文章或書摘'); return }
    if (!apiKey.trim() && !proxyMode) { setGenError('請輸入 Gemini API Key'); return }
    setGenerating(true)
    setGenError('')
    const systemPrompt = `你是一位優秀的讀書會引導師。請根據以下文章內容，設計 2 道能激發深度討論的開放性思辨問題。
要求：
...
${content.slice(0, 3000)}`
    ...
    try {
      ...
      if (proxyMode && proxyDomain) {
        url = `https://${proxyDomain}/gemini/v1beta/models/gemini-1.5-flash:generateContent`
        headers = { ...headers }
      } else {
      ...
      if (!Array.isArray(parsed) || parsed.length < 2) throw new Error('回應格式異常')
  ...
  async function createRoom() {
    ...
    try {
      await setDoc(doc(db, sessionsPath(code)), {
        title: title.trim(),
  ...
  async function togglePublish(answerId, current) {
    try {
      await updateDoc(doc(db, `${answersColPath}/${answerId}`), { isPublished: !current })
    } catch (e) {
      console.error(e)
    }
  }
  ...
  function handleCopy() {
    const ok = copyRoomCode(roomCode)
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-800 rounded-xl p-5 space-y-4">
              <h2 className="text-white font-semibold">AI 問題生成設定</h2>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span className="text-slate-300 text-sm">使用中繼代理（適合中國大陸用戶）</span>
              </label>

              {proxyMode ? (
                <div>
                  <label className="block text-slate-400 text-sm mb-1">Cloudflare Worker 代理網域</label>
                  <input
                    type="text"
                    value={proxyDomain}
                    onChange={e => setProxyDomain(e.target.value)}
                    placeholder="your-worker.workers.dev"
                    className="w-full bg-slate-700 text-white placeholder-slate-500 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500"
                  />
                </div>
              ) : (
                <div>
                  <label className="block text-slate-400 text-sm mb-1">Gemini API Key</label>
                  <input
                    type="password"
                    value={apiKey}
                    onChange={e => setApiKey(e.target.value)}
                    placeholder="AIza..."
                    className="w-full bg-slate-700 text-white placeholder-slate-500 rounded-lg px-4 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-violet-500"
                  />
                </div>
              )}
              ...
              <button
                ...
                disabled={generating}
                className="w-full py-3 bg-violet-600 hover:bg-violet-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-colors"
              >
            ...
            {questions.length === 2 && (
              <div className="bg-slate-800 rounded-xl p-5 space-y-4 border border-violet-800">
                <h2 className="text-violet-300 font-semibold flex items-center gap-2">
                  <span>✦</span> AI 生成的思辨問題
                </h2>
                {questions.map((q, i) => (
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={onBack} className="text-slate-400 hover:text-white transition-colors text-sm">← 返回首頁</button>
        </div>
          ...
          {displayAnswers.length === 0 ? (
            ...
            <div className="space-y-3">
              {displayAnswers.map(a => (
                <div
                  key={a.id}
                  className={`bg-slate-800 rounded-xl p-4 border transition-colors ${a.isPublished ? 'border-teal-700' : 'border-slate-700'}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {a.isPublished && (
                          <span className="px-2 py-0.5 bg-teal-900 text-teal-300 text-xs rounded-full">已公開</span>
                        )}
                      </div>
